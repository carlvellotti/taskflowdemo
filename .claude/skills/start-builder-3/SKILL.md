...
- **Student runs `/clear` mid-lesson anyway:** They can rerun `/start-builder-3`; the lesson is idempotent (no asset deploy, no lesson-state writes), so pick up from wherever their app state actually is.
