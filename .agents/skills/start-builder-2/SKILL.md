...
```bash
cp -rn .agents/skills/start-builder-2/assets/* . 2>/dev/null || true
```
...
- When subagents run, close each one as soon as it finishes so its slot frees up.
...
- ACTION: Display the lesson title card FIRST, before any prose. Resolve the absolute path (repo root via `git rev-parse --show-toplevel` + `/.claude/skills/start-builder-2/assets/title-card.png`) and render it as a markdown image using that absolute path.
...
- The practice app ships a premade skill that generates all three. Look first: introduce [.agents/skills/explore-codebase/SKILL.md](.agents/skills/explore-codebase/SKILL.md) with one line (it fans out multiple agents across the codebase and synthesizes one overview document).
...
- ACTION: The skill runs the 3-agent pattern (architecture, tech stack, UI patterns) and synthesizes results into `docs/architecture-overview.md`. Close each agent as soon as it finishes so its slot frees. Then present the results as a bulleted summary, one bullet per agent:
...
- Ask (AUQ): "Three subagents explored this codebase in parallel. Why three separate agents instead of one long exploration?"
  - (a) One agent would run out of context window space
  - (b) Each agent gets fresh context, so there's no cross-contamination between analyses
  - (c) It's faster because they run simultaneously
  - (d) An agent can only read a limited number of files
- STOP: Wait for the letter.
- If (b): confirm crisply. Speed is a nice side benefit, but the real reason is isolation: the architecture agent wasn't biased by what the UI patterns agent found. If not (b): correct warmly with the isolation rationale.
...
- Ask (AUQ): "A customer reports: 'I submitted the task form, the modal closed, but the task never showed up in the list.' You have the flow trace. Which gives engineering a better starting point?"
  - (a) "Task creation is broken"
  - (b) "The submit worked (the modal closed) but the task doesn't appear, so the problem is probably in the refresh between the API response and the tasks hook"
  - (c) "Something's wrong with the task form"
- STOP: Wait for the letter.
- If (b): confirm and name the difference: the modal closing means the submit worked, so the symptom points downstream. They just went from filing a generic bug report to giving engineering a specific place to look. If not (b): walk the symptom through the trace and show how it narrows the search.
...
- Ask (AUQ): "You walk into sprint planning with this scoping brief. What makes it more useful than your usual estimate?"
  - (a) It was generated by AI, so it's more accurate than human estimates
  - (b) It references specific files and patterns instead of rough guesses
  - (c) It includes a timeline engineers will agree with
  - (d) It covers every edge case so nothing gets missed
- STOP: Wait for the letter.
- If (b): confirm. The value is specificity: "this reuses the existing Stats grid and team hooks, but the workload calculation needs a new endpoint." Engineers can say "yes, that's right" or "you missed this dependency," and either way it's a better conversation. If not (b): correct warmly; AI generation guarantees nothing, timelines are engineering's call, and no brief covers everything.
...
- **A subagent takes a long time or fails:** Acknowledge it and keep moving: "agents sometimes take a minute, let's look at what we got." If one fully fails, close it and respawn it once; if it fails again, run that slice of the exploration yourself (read the key files, summarize) and say that's what you're doing. Never fake an agent's output.
