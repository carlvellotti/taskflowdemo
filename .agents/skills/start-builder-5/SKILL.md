...
- STOP: Wait for them to send it.
- ACTION: Run `gh repo create` for a new repo under the student's account (public or private, their choice; ask if they have a preference), then update the git remote to point at it. If auth problems come up, walk through `gh auth login` (GitHub.com, HTTPS, authenticate via browser).
...
- **Push fails:** Most likely the remote still points at the course's repo (no write access, and that failure is expected: it's exactly why they create their own repo) or auth. Check `git remote -v`, re-run the repo-create beat if needed, and walk `gh auth login` (GitHub.com, HTTPS, browser) for auth.
- **`gh repo create` or PR creation fails:** Almost always auth. Walk them through `gh auth login` step by step; be patient, it may be their first CLI auth. If `gh` isn't installed at all, `brew install gh` on Mac or cli.github.com, then auth.
