...
**Step 1: Launch three sub-agents in parallel** by spawning subagents. Each agent gets its own fresh context, no cross-contamination between analyses. Close each agent when it finishes so its slot frees.
